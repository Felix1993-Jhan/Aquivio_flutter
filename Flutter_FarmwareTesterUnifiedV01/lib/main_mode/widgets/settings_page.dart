...
class _SettingsPageState extends State<SettingsPage> {
          ...
          children: [
            ...
            Row(
              ...
              children: [
                // 左側：Arduino 設定
                Expanded(
                  child: _buildDeviceThresholdColumn(
                    deviceName: 'Arduino',
                    color: EmeraldColors.primary,
                    darkColor: EmeraldColors.dark,
                    device: DeviceType.arduino,
                  ),
                ),
                ...
                Expanded(
                  child: _buildDeviceThresholdColumn(
                    ...
                    darkColor: SkyBlueColors.dark,
                    device: DeviceType.stm32,
                  ),
                ),
              ],
            ),
  ...
  /// 建構單一設備的閾值設定欄
  Widget _buildDeviceThresholdColumn({
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // 感測器閾值
          _buildSensorThresholdTile(
            title: tr('sensor_threshold'),
            color: color,
            device: device,
          ),
        ],
  ...
  Widget _buildThresholdCategoryTile({
    ...
    return Container(
      ...
      child: ListTile(
        ...
        onTap: () => _showThresholdEditDialog(device, state, title, color),
      ),
    );
  }

  /// 建構感測器閾值磚塊
  Widget _buildSensorThresholdTile({
    required String title,
    required Color color,
    required DeviceType device,
  }) {
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(title, style: const TextStyle(fontWeight: FontWeight.w500)),
        subtitle: Text(
          'ID 18-${device == DeviceType.arduino ? 21 : 23}',
          style: TextStyle(fontSize: 12, color: Colors.grey.shade600),
        ),
        trailing: const Icon(Icons.chevron_right),
        onTap: () => _showSensorThresholdEditDialog(device, title, color),
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (context) => _ThresholdEditDialog(
        ...
        onApplyToAll: (range) async {
          await _thresholdService.setAllHardwareThresholds(device, state, range);
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(tr('save_success'))),
            );
  ...
  /// 顯示感測器閾值編輯對話框
  void _showSensorThresholdEditDialog(DeviceType device, String title, Color color) {
    final thresholds = _thresholdService.getAllSensorThresholds(device);

    showDialog(
      context: context,
      builder: (context) => _SensorThresholdEditDialog(
        title: title,
        color: color,
        device: device,
        thresholds: thresholds,
        onSave: (newThresholds) async {
          for (final entry in newThresholds.entries) {
            await _thresholdService.setSensorThreshold(device, entry.key, entry.value);
          }
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text(tr('save_success'))),
...
class _ThresholdEditDialogState extends State<_ThresholdEditDialog> {
  ...
  void _showSingleEditDialog(int id) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('${tr('edit_threshold')} - ID $id'),
        content: Row(
          children: [
            Expanded(
              child: TextField(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (min != null && max != null && min <= max) {
                setState(() {
                  _editedThresholds[id] = ThresholdRange(min: min, max: max);
                });
                Navigator.pop(context);
              }
            },
...
class _SensorThresholdEditDialog extends StatefulWidget {
  final String title;
  final Color color;
  ...
  const _SensorThresholdEditDialog({
    required this.title,
    required this.color,
  ...
  @override
  State<_SensorThresholdEditDialog> createState() => _SensorThresholdEditDialogState();
}

class _SensorThresholdEditDialogState extends State<_SensorThresholdEditDialog> {
  late Map<int, ThresholdRange> _editedThresholds;
  ...
  @override
  Widget build(BuildContext context) {
    final maxId = widget.device == DeviceType.arduino ? 21 : 23;
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    title: Text(DisplayNames.getName(id)),
                    subtitle: Text('${range?.min ?? 0} ~ ${range?.max ?? 0}'),
                    trailing: IconButton(
                      icon: const Icon(Icons.edit, size: 18),
                      onPressed: () => _showSingleEditDialog(id),
                    ),
                  );
                },
              ),
